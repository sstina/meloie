...
| `--precision` | `fp32` | Inference precision. FP32 uses a 1 s reflect-pad vs FP16's 3 s → ~25% faster (lower inference floor) and measured spectrally identical to FP16; `auto`/`fp16` available. Pure precision, no voice change |
| `--chunk-ms` | 500 | RVC chunk size (accumulation latency); larger = more model context. Floor ~300 (FP32): worst-case inference (~260 ms) is roughly fixed regardless of chunk |
...
~105 ms inference (FP32) + ~40 ms device ≈ **~1.5 s** at the defaults. The standing
output prebuffer used to dominate (it defaulted to 3 × chunk = 3 s); decoupling it to
an absolute 800 ms and dropping the chunk to 500 ms brought the link down from ~2 s,
and FP32 cut inference (mean ~105 ms, max ~260 ms) below the old FP16 floor. Going
lower means a smaller chunk — FP32's ~260 ms worst case now allows ~300 ms; true
sub-second would require pacing the output burst (a deferred code change), not just
smaller buffers. None of these touch the voice — see the design stance above.
